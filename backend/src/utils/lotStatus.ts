...
export const updateLotStatus = async (prisma: Prisma.TransactionClient, lotId: string) => {
  try {
      ...
      // If lot is cancelled, notify seller
        ...
        await prisma.notification.create({
          data: {
            userId: lot.sellerId,
            type: 'LOT_CANCELLED',
            message: `Ваш лот "${lot.title}" был отменен, так как не было поставлено ни одной ставки`,
            lotId: lot.id
          }
        });
      }
